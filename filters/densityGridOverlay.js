...
/**
 * The DensityGridOverlay class with a fallback if pointInPolygon fails.
 */
export class DensityGridOverlay {
    ...
    for (let x = -halfExt; x <= halfExt; x += this.gridSize) {
      for (let y = -halfExt; y <= halfExt; y += this.gridSize) {
        for (let z = -halfExt; z <= halfExt; z += this.gridSize) {
          ...
          const squareGlobe = new THREE.Mesh(planeGeom, mat2);

          if (dist < 1e-6) {
            squareGlobe.position.set(0,0,0);
          } else {
            const ra = Math.atan2(-posTC.z, -posTC.x);
            const dec = Math.asin(posTC.y / dist);
            const R = 100;
  ...
  // The BFS cluster classification
  classifyEmptyRegions() {
    ...
    clusters.forEach((cells,idx)=>{
      ...
      if(regionType==="Ocean" && segResult.segmented){
        // re-check local freq for each sub-sea
        ...
        if(segResult.neck && segResult.neck.length>0){
          ...
          segResult.neck.forEach(c=>{
            if(c.constellation!=="UNKNOWN"){
              neckFreq[c.constellation]=(neckFreq[c.constellation]||0)+1;
            }
          });
          let localMajority="UNKNOWN";
          let localMax=0;
          for(const nm in neckFreq){
            if(neckFreq[nm]>localMax){
              localMax=neckFreq[nm];
              localMajority=nm;
            }
          }
          regions.push({
  ...
  /**
   * The refined method: we still do pointInPolygon, but if no match, we pick the nearest polygon by centroid.
   */
  assignConstellationsToCells(constellationData) {
    this.cubesData.forEach(cell => {
      ...
      // 1) attempt normal pointInPolygon
      let foundConst = null;
      ...
      if (foundConst) {
        cell.constellation = foundConst.trim().toUpperCase();
        console.log(`Cell ID ${cell.id} => assigned constellation [${cell.constellation}] (direct polygon match)`);
        return;
      }

      // 2) fallback => pick the polygon whose centroid is nearest in RA/DEC
      let bestObj = null;
      let bestDist = 999999;
      for (const cObj of constellationData) {
        const cCent = computePolygonCentroid(cObj.raDecPolygon);
        const distDeg = sphericalDistance(cellRaDec, cCent);
        if (distDeg < bestDist) {
          bestDist = distDeg;
          bestObj = cObj;
        }
      ...
      if (bestObj) {
        cell.constellation = bestObj.constellation.trim().toUpperCase();
        console.log(`Cell ID ${cell.id} => assigned constellation [${cell.constellation}] (fallback near ${bestDist.toFixed(2)}°)`);
      } else {
        // extremely unlikely to fail entirely
        cell.constellation = "UNKNOWN";
      }
    });
  }
...
/**
 * Basic pointInPolygon test in RA/DEC
 */
...
/**
 * Fallback approach: measure spherical distance between two RA/DEC points in degrees
 */
function sphericalDistance(a, b){
  // simple haversine on the unit sphere
  const degToRad=Math.PI/180;
  const lat1=a.dec*degToRad, lat2=b.dec*degToRad;
  const dLat=lat2-lat1;
  let dLon=(b.ra - a.ra)*degToRad;
  ...
  const aa=sLat*sLat + Math.cos(lat1)*Math.cos(lat2)*sLon*sLon;
  const c=2*Math.atan2(Math.sqrt(aa),Math.sqrt(1-aa));
  return c*(180/Math.PI); // return degrees
}

/**
 * Compute approximate centroid for a polygon in RA/DEC
 * (just the average, mindful of wrap-around issues)
 */
function computePolygonCentroid(raDecPoly){
  // naive approach: average RA, DEC in degrees
  // we can do something more robust for crossing 0/360 boundary, but let's keep it simple
  if(!raDecPoly || raDecPoly.length===0){
    return { ra:0, dec:0 };
  }
  let sumRA=0;
  let sumDec=0;
  raDecPoly.forEach(pt=>{
    sumRA+=pt.ra;
    sumDec+=pt.dec;
  });
  const n=raDecPoly.length;
  return { ra: sumRA/n, dec: sumDec/n };
}
